...
class Order {
  ...
  public function __construct(AuthToken $auth) {
    ...
    foreach ($titles as $title) {
      ...
      DB::upd(array('_id' => $id), array('$set' => array('status' => 'pending', 'lastStatusChange' => new \MongoDB\BSON\UTCDateTime(time()))), 'titles');
    }
   ...
   *
   * @param string|false $dir Directory to create the temp dir in
