...
class QueryValidator {
   ...
   */
  public function __construct($query, $type = 'keyword') {
    ...
    if ($type === 'advanced') {
      ...
      for ($i = 0; $i < count($query); $i++) {
        ...
        if (!is_array($criterium) || empty($criterium['field']) || empty($criterium['mode'] || !is_set($criterium['value']))) {
          throw new InvalidQueryException('Fehlende Informationen in einem Suchkriterium');
        }

        if (!in_array($criterium['mode'], array_keys(Config::$search_modes))) {
      ...
      if (preg_match('/([a-z]{3}) (.*)/', $query, $matches) && in_array($matches[1], array_keys(Config::$searchable_fields))) {
